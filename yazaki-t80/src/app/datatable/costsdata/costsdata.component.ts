...
export class CostsdataComponent implements OnInit {
  ...
  index: number;
  id: number;
  displayedColumns = ['date_week', 'users.name', 'department', 'totalcoasts', 'totalamount', 'approved', 'actions'];
  constructor(public httpClient: HttpClient,
    public dialog: MatDialog, private costservice: CostsService, private router: Router) { }
  ...
  view($id){
    console.log($id);
    this.router.navigateByUrl('/Viewcost/'+$id);
  }
  ...
  connect(): Observable<Cost[]> {
    ...
    return merge(...displayDataChanges).pipe(map(() => {
      ...
      this.filteredData = this.costService.data.slice().filter((User: Cost) => {
        ...
        const searchStr = (User.date_week + User.users.name + User.department + User.approved).toLowerCase();
