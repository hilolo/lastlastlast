...
export class UserComponent implements  OnInit {
  ...
  displayedColumns = [ 'email', 'name','email2','operation', 'actions'];
  constructor(public httpClient: HttpClient,  private auth: AuthapiService,
  ...
  opend(email: string){
    ...
    this.auth.emailinvite(email).subscribe(
      data => this.handleResponse(data,email)
  ...
  handleResponse(data,email)
  {
    Swal.fire({
      position: 'top-end',
      icon: 'success',
      title: ' invitation has ben sented to ' +email +'  succesfully!',
